;(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined'
    ...
    : typeof define === 'function' && define.amd
      ...
      : ((global =
        ...
        factory((global.docx = {}), global.JSZip))
  ...
  function parseXmlString(xmlString, trimXmlDeclaration = false) {
    ...
    // xhr.send()
    //   const str = `<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
    //   <Relationship Id="rId3" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/extended-properties" Target="docProps/app.xml"/>
    //   <Relationship Id="rId2" Type="http://schemas.openxmlformats.org/package/2006/relationships/metadata/core-properties" Target="docProps/core.xml"/>
    //   <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="word/document.xml"/>
    // </Relationships>`
    debugger
    const str = '<div>assd</div>'
    const rr = new DOMParser().parseFromString(str, 'text/html')
    console.log(rr.documentElement)
  ...
  class OpenXmlPackage {
    ...
    async loadRelationships(path = null) {
      ...
      return txt
        ? parseRelationships(
            this.parseXmlDocument(txt).firstElementChild,
            this.xmlParser
          )
        : null
    }
    ...
    for (let e of xml.elements(elem)) {
      switch (e.localName) {
        ...
        case 'headerReference':
          ;(section.headerRefs ?? (section.headerRefs = [])).push(
            parseFooterHeaderReference(e, xml)
          )
          break
        case 'footerReference':
          ;(section.footerRefs ?? (section.footerRefs = [])).push(
            parseFooterHeaderReference(e, xml)
          )
          break
        case 'titlePg':
    ...
    async loadRelationshipPart(path, type) {
      ...
      switch (type) {
        ...
        case RelationshipTypes.Comments:
          this.commentsPart = part = new CommentsPart(
            this._package,
            path,
            this._parser
          )
        ...
        case RelationshipTypes.CommentsExtended:
          this.commentsExtendedPart = part = new CommentsExtendedPart(
            this._package,
            path
          )
          break
  ...
  function parseStroke(el) {
    return {
      ...
      'stroke-width':
        globalXmlParser.lengthAttr(el, 'weight', LengthUsage.Emu) ?? '1px'
    }
  }
      ...
      xmlUtil.foreach(xnums, (n) => {
        switch (n.localName) {
          ...
          case 'num':
            ...
            var abstractNumId = globalXmlParser.elementAttr(
              n,
              'abstractNumId',
              'val'
            )
        ...
        switch (el.localName) {
          ...
          case 'commentRangeStart':
            result.children.push(
              new WmlCommentRangeStart(globalXmlParser.attr(el, 'id'))
            )
            break
          case 'commentRangeEnd':
            result.children.push(
              new WmlCommentRangeEnd(globalXmlParser.attr(el, 'id'))
            )
            break
          case 'oMath':
          case 'oMathPara':
          ...
          case 'sdt':
            result.children.push(
              ...this.parseSdt(el, (e) => this.parseParagraph(e).children)
            )
            break
          case 'ins':
            result.children.push(
      ...
      xmlUtil.foreach(node, (c) => {
        ...
        switch (c.localName) {
          ...
          case 'commentReference':
            result.children.push(
              new WmlCommentReference(globalXmlParser.attr(c, 'id'))
            )
            break
          case 'fldSimple':
            result.children.push({
      ...
      for (var n of globalXmlParser.elements(node)) {
        switch (n.localName) {
          case 'simplePos':
            ...
            break
          case 'extent':
            result.cssStyle['width'] = globalXmlParser.lengthAttr(
              n,
              'cx',
              LengthUsage.Emu
            )
            result.cssStyle['height'] = globalXmlParser.lengthAttr(
              n,
              'cy',
              LengthUsage.Emu
            )
      ...
      for (var n of globalXmlParser.elements(xfrm)) {
        switch (n.localName) {
          case 'ext':
            ...
            break
          case 'off':
            result.cssStyle['left'] = globalXmlParser.lengthAttr(
              n,
              'x',
              LengthUsage.Emu
            )
            result.cssStyle['top'] = globalXmlParser.lengthAttr(
              n,
              'y',
              LengthUsage.Emu
            )
        ...
        switch (c.localName) {
          ...
          case 'sz':
            style['font-size'] = style['min-height'] =
              globalXmlParser.lengthAttr(c, 'val', LengthUsage.FontSize)
            break
          case 'shd':
          ...
          case 'vertAlign':
            break
          case 'position':
            style.verticalAlign = globalXmlParser.lengthAttr(
              c,
              'val',
  ...
  class HtmlRenderer {
    ...
    renderHeaderFooter(refs, props, page, firstOfSection, into) {
      ...
      var part =
        ref && this.document.findPartByRelId(ref.id, this.document.documentPart)
      if (part) {
        ...
        if (elem.type == DomType.Paragraph) {
          ...
          if (this.options.breakPages && p.children) {
            pBreakIndex = p.children.findIndex((r) => {
              rBreakIndex =
                r.children?.findIndex(this.isPageBreakElement.bind(this)) ?? -1
...
`
    ...
    renderCommentRangeStart(commentStart) {
      ...
      const result = this.htmlDocument.createComment(
        `start of comment #${commentStart.id}`
      )
    ...
    renderCommentReference(commentRef) {
      ...
      const commentRefEl = createElement(
        'span',
        { className: `${this.className}-comment-ref` },
        ['💬']
      )
      const commentsContainerEl = createElement('div', {
    ...
    renderCommentContent(comment, container) {
      ...
      container.appendChild(
        createElement('div', { className: `${this.className}-comment-date` }, [
    ...
    renderMmlDelimiter(elem) {
      const children = []
      children.push(
        createElementNS(ns.mathML, 'mo', null, [elem.props.beginChar ?? '('])
      )
    ...
    renderMmlNary(elem) {
      ...
      if (supElem || subElem) {
        ...
        children.push(
          createElementNS(ns.mathML, 'mover', null, [charElem, supElem])
        )
      } else if (subElem) {
        children.push(
          createElementNS(ns.mathML, 'munder', null, [charElem, subElem])
        )
      } else {
    ...
    renderMmlPreSubSuper(elem) {
      ...
      children.push(
        createElementNS(ns.mathML, 'msubsup', null, [
          stubElem,
          subElem,
          supElem
        ])
    ...
    renderMmlGroupChar(elem) {
      const tagName =
        elem.props.verticalJustification === 'bot' ? 'mover' : 'munder'
      const result = this.renderContainerNS(elem, ns.mathML, tagName)
      if (elem.props.char) {
        result.appendChild(
          createElementNS(ns.mathML, 'mo', null, [elem.props.char])
        )
      }
      return result
    }
    ...
    renderMllList(elem) {
      ...
      for (let child of this.renderChildren(elem)) {
        result.appendChild(
          createElementNS(ns.mathML, 'mtr', null, [
